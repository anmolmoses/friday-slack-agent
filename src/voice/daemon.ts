...
function engineeringContext(cfg: ReturnType<typeof loadVoiceConfig>): string {
  ...
  return [
    ...
    "Use local Codex for engineering dispatch by default. Use Slack/Claude dispatch only if Anmol explicitly asks for the cloud/Slack/Claude route.",
    "Configured repos:",
    repos,
  ].join("\n");
}
...
export async function runDaemon(opts: {
  ...
  function extractFavoriteSong(memoryText: string): string | undefined {
    ...
    const match =
      cleaned.match(
        /\bAnmol(?:'s)? favorite song (?:is|=)\s+(.+?)(?:\.|;|,?\s+When\b|$)/i,
      ) ??
      cleaned.match(
      ...
      if (images.length > 0) {
        ...
        client.createResponse(
          continueSilently
            ...
            : currentInstructions(
                "A screen or camera image is attached. Answer Anmol's visual request aloud in one complete sentence of eight words or fewer. Name the main app, object, person, or UI state. End with a period. Do not say the task is handled. Do not call tools or read file paths.",
              ),
          {
            queueIfActive: true,
        ...
        if (cfg.aiDriven) {
          client.sendFunctionResult(callId, output, true, {
            instructions: currentInstructions(
              "The tool finished and its result is in the function output above. Tell Anmol the real outcome in one short spoken sentence, grounded in that output. If it shows an error or the task is not actually done, say so plainly — never claim a success you cannot see in the output.",
            ),
            maxOutputTokens: cfg.maxOutputTokens,
            toolChoice: "none",
            outputModalities: ["audio"],
