...
describe("voice action routing", () => {
  ...
  it("fast-paths clear engineering work to local Codex dispatch", () => {
    ...
    expect(deterministicAction("fix the typecheck in gx-backend")).toEqual({
      name: "dispatch_engineering",
      args: {
        prompt: "fix the typecheck in gx-backend",
        engine: "auto",
      },
    });
    ...
    expect(deterministicAction("check the logs in gx-backend")).toEqual({
      name: "dispatch_engineering",
      args: {
        prompt: "check the logs in gx-backend",
        engine: "auto",
      },
    });
    expect(
  ...
  it("prefers the right Realtime tool for non-direct actions", () => {
    ...
    expect(preferredToolForAction("fix the typecheck in gx-backend")).toBe(
