 ...
 *
 *   2. recentJabs — non-Anmol messages in vibes channels that look like
 ...
 *
 * The Prickle thread (#cafeteria, 2026-04-01) is the canonical scar — ~20
 * self-deprecating posts chasing Pranav's bait. The spiral IS the bait.
 */
...
export function spiralBrakeFragment(): string {
  return [
    "## ⚠️ SPIRAL DETECTED",
    "Your recent replies in this thread are stacking self-deprecation markers (`pathetic`, `i'm done`, `friday out`, `not taking the bait`, `goodnight`, etc.). The Prickle thread (#cafeteria, 2026-04-01) is the scar — ~20 of these in a row, every one a point scored on you.",
    "Next reply rules:",
...
/**
 * Classify a non-Anmol vibes-channel message as a jab or not. A jab is a
...
export function classifyJab(input: {
  ...
  // A naked @Friday in a vibes channel from a non-Anmol user is mild but
