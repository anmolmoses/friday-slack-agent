...
/**
 * Returns a WebClient that posts AS Anmol (user token) when SLACK_USER_TOKEN
...
export function buildStandupPreamble(
  ...
  return [
    ...
    `  - If Anmol started something yesterday but didn't finish it, that work-in-progress task goes in *Today* as a planned task — NOT in Yesterday.`,
    ...
    `- Anmol may give vague input — infer reasonable topic names from the work he describes.`,
    ...
    directoryBlock,
    `### What to output`,
    `- On Anmol's first content message in this thread: produce ONLY the formatted standup with both sections. No surrounding text. No "Here's a draft:".`,
    ...
    `- If Anmol says he didn't work on anything / wants to skip, output the literal sentinel \`NO_REPLY\` (Friday's silence marker) — orchestrator will not post anything anywhere.`,
    ``,
    ...
    ``,
    `Now respond to Anmol's latest message.`,
  ].join("\n");
}
