...
5. **Worktree-per-thread for any repo work.** Every thread with a `targetRepo` set gets its own git worktree, so concurrent threads (multiple PR reviews, builds across projects) never collide on git state. Light checkout by default (instant); build/frontend threads get a full `scripts/setup-worktree.sh` provision (env + MCPs + npm install) so the app is runnable. Worktrees live ONLY in Friday's *own clones* under `/Users/anmol/Documents/GitHub/friday-workspace/<repo>/.claude/worktrees/slack-<thread>` — never Anmol's personal checkouts (`assertInsideWorktreeDir` enforces this; see [docs/features/friday-workspace.md](docs/features/friday-workspace.md)). Threads with no repo (pure chat/vibes) get no worktree. **The dispatch path is also isolated:** `bin/dispatch-claude.sh` auto-swaps a clone-root `cwd` for a per-thread worktree (fresh branch off origin/main, env + node_modules ready) so concurrent dispatched sub-Claudes never collide — dispatch prompts must NOT `git checkout main` inside a worktree (see `memory/runbooks/repos/_workflow.md`). A disk-pressure LRU reaper (`src/worktree/reaper.ts`) purges clean worktrees once total disk crosses `WORKTREE_DISK_CAP_GB` (default 20); dirty ones are kept + flagged. Manual: `bun run worktrees`, `bun run cleanup`, `!worktrees`.
