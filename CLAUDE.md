...
friday is a Slack bot that acts as the control plane for Claude Code sessions. It's the successor to an OpenClaw-based agent system — same role (Friday the orchestrator), rebuilt on Claude Code as a subprocess instead of OpenClaw.
...
This project replaces an earlier OpenClaw-based agent workspace. Key things that carry over:
...
- **Build -> Review loop:** Build via agent -> push -> Bones reviews -> fix -> re-review -> ship. 3-round escalation to the human owner if Bones keeps finding blockers.
...
# Read or reply to ANY Slack message by URL (limited to your bot's workspace — uses SLACK_BOT_TOKEN)
