#!/usr/bin/env bash
# PreToolUse hook — blocks non-Anmol Slack users from making Friday modify
# her own source code. Allows everything for Anmol (U09SZ4DM8TH); allows
...
ANMOL_USER_ID="U09SZ4DM8TH"
FRIDAY_ROOT="/Users/anmol/Documents/GitHub/Friday"
...
# Anmol has full access.
if [ "${SLACK_USER_ID:-}" = "$ANMOL_USER_ID" ]; then
  exit 0
fi
...
block() {
  ...
  echo "Blocked: only Anmol (U09SZ4DM8TH) can mutate Friday's source code (${FRIDAY_ROOT}). Requester=${who}. ${why}" >&2
...
case "$TOOL" in
  ...
  Bash)
    ...
    # 2. `cd /Users/anmol/Documents/GitHub/Friday && <git-write-or-shell-write>`
    ...
    # strict here because we already know this is non-Anmol and the working
    # directory IS Friday.
