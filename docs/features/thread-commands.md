...
## Design Decision: Inline prefix, not Slack slash commands

Using Slack slash commands (`/junior-build`) requires registering each command with Slack's API, adds latency (Slack's command dispatch), and limits us to predefined commands. Instead, parse commands from message text:

- Message starts with `/` → extract command + remaining text
- Message doesn't start with `/` → entire message is the prompt

This keeps everything in Slack's normal message flow — no special API registration, no callback URLs.
